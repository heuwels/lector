#!/usr/bin/env python3
"""Build the Norwegian Tatoeba cloze bank used by the API.

The output is derived from Tatoeba's `nob` -> `eng` per-language exports
...
Prerequisites:
    ...
    npx tsx scripts/build-dictionary.ts --lang nb

Usage:
    python scripts/build-cloze-nb.py
    python scripts/build-cloze-nb.py --max-words 2000 --sentences-per-word 6

Downloads are cached in tmp/cloze-nb. Re-running with the same Tatoeba exports,
...
NORWEGIAN_WORD = re.compile(r'^[a-zæøå-]+$')
APOSTROPHES = re.compile(r"['‘’ʼ`]+")

# Keep this aligned with languages/nb/manifest.ts.
AVOID_WORDS = {
    'en', 'ei', 'et', 'den', 'det', 'de', 'denne', 'dette', 'disse',
    'jeg', 'du', 'han', 'hun', 'vi', 'dere', 'man', 'seg', 'meg', 'deg',
    'oss', 'ham', 'henne', 'dem',
    'min', 'mitt', 'mine', 'din', 'ditt', 'dine', 'hans', 'hennes', 'dens',
    'dets', 'vår', 'vårt', 'våre', 'deres', 'sin', 'sitt', 'sine',
    'i', 'på', 'av', 'til', 'fra', 'for', 'med', 'om', 'under', 'over',
    'etter', 'før', 'mellom', 'uten', 'mot', 'ved', 'hos', 'gjennom',
    'og', 'eller', 'men', 'så', 'at', 'når', 'fordi', 'mens', 'samt', 'som',
    'er', 'var', 'vært', 'være', 'har', 'hadde', 'ha', 'kan', 'kunne',
    'skal', 'skulle', 'vil', 'ville', 'må', 'får', 'fikk', 'blir',
    'ble', 'bli',
    'ikke', 'ja', 'nei', 'jo',
    'hva', 'hvem', 'hvilken', 'hvilket', 'hvilke', 'hvor', 'hvordan', 'hvorfor',
    'også', 'bare', 'mye', 'mer', 'mest', 'her', 'der', 'nå',
    'da', 'allerede', 'ennå',
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dictionary',
        type=Path,
        default=PROJECT_ROOT / 'data' / 'dictionary-nb.db',
        help='Norwegian dictionary database built by scripts/build-dictionary.ts',
    )
    parser.add_argument(
        '--cache-dir',
        type=Path,
        default=PROJECT_ROOT / 'tmp' / 'cloze-nb',
        help='Tatoeba download/decompression cache',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT / 'api' / 'src' / 'lib' / 'sentence-bank-nb.json',
...
def build_candidates(dictionary_path: Path, max_words: int) -> list[Candidate]:
    if not dictionary_path.exists():
        raise FileNotFoundError(
            f'{dictionary_path} does not exist; run the Norwegian dictionary build first'
        )
    ...
    try:
        ...
        for raw_word in top_n_list('nb', max(max_words * 5, 10000)):
            word = normalize(raw_word).lower()
            if (
                word in seen
                or word in AVOID_WORDS
                or len(word) < 2
                or not NORWEGIAN_WORD.fullmatch(word)
...
def collect_options(
    ...
    for sentence_id in sorted(norwegian):
        text = norwegian[sentence_id]
...
def main() -> int:
    ...
    print('=== Norwegian Tatoeba cloze builder ===')
    candidates = build_candidates(args.dictionary, args.max_words)
    norwegian_path, links_path, english_path = ensure_tatoeba_files(args.cache_dir)

    norwegian = load_sentences(norwegian_path)
    links, needed_english = load_links(links_path, set(norwegian))
    english = load_english(english_path, needed_english)
    print(
        f'Tatoeba: {len(norwegian)} length-filtered Norwegian sentences; '
        f'{len(links)} linked; {len(english)} English translations'
    )

    options = collect_options(norwegian, links, english, candidates)
