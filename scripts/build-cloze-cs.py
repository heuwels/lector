#!/usr/bin/env python3
"""Build the Czech Tatoeba cloze bank used by the API.

The output is derived from Tatoeba's `ces` -> `eng` per-language exports
...
Prerequisites:
    ...
    npx tsx scripts/build-dictionary.ts --lang cs

Usage:
    python scripts/build-cloze-cs.py
    python scripts/build-cloze-cs.py --max-words 2000 --sentences-per-word 6

Downloads are cached in tmp/cloze-cs. Re-running with the same Tatoeba exports,
...
# Plain Czech letter runs only: a-z plus the háček letters, the acute letters
# and ů. Hyphenated compounds (česko-slovenský, modro-bílá) stay whole tokens at
# runtime and in sentence_matches below, so they can never collide with a plain
# candidate; wordfreq emits unhyphenated tokens anyway.
CZECH_WORD = re.compile(r'^[a-záčďéěíňóřšťúůýž]+$')
APOSTROPHES = re.compile(r"['‘’ʼ`]+")

# Keep this aligned with languages/cs/manifest.ts. The POS filter removes most
...
AVOID_WORDS = {
    # prepositions (both the plain and the vocalized spelling: v/ve, z/ze, s/se)
    'v', 've', 'z', 'ze', 's', 'se', 'k', 'ke', 'na', 'do', 'od', 'ode',
    'pro', 'přes', 'po', 'při', 'o', 'u', 'za', 'pod', 'nad', 'před', 'mezi',
    'bez', 'kromě', 'vedle', 'kolem', 'okolo', 'podle', 'během', 'proti',
    'kvůli', 'díky', 'místo',
    # conjunctions and connectives
    'a', 'i', 'ale', 'avšak', 'nebo', 'anebo', 'či', 'že', 'aby', 'když',
    'jestli', 'jestliže', 'pokud', 'protože', 'neboť', 'tedy', 'tak', 'také',
    'též', 'však', 'ač', 'ačkoli', 'ani', 'než', 'jak',
    # particles
    'ne', 'si', 'by', 'už', 'ještě', 'jen', 'jenom', 'velmi', 'dokonce',
    'ano', 'právě', 'možná', 'asi', 'snad', 'ať', 'prý', 'no',
    # personal, reflexive and possessive pronouns (case forms)
    'já', 'ty', 'on', 'ona', 'ono', 'my', 'vy', 'oni', 'ony',
    'mě', 'mne', 'mi', 'mnou', 'tebe', 'tě', 'tobě', 'ti', 'tebou',
    'jeho', 'ho', 'jemu', 'mu', 'něm', 'něj', 'ním',
    'jí', 'ji', 'ní',
    'nás', 'nám', 'námi', 'vás', 'vám', 'vámi', 'jich', 'jim', 'jimi', 'nich',
    'sebe', 'sobě', 'sebou',
    'můj', 'moje', 'má', 'mé', 'tvůj', 'tvoje', 'tvá',
    'náš', 'naše', 'váš', 'vaše', 'jejich', 'svůj', 'svá', 'své',
    # demonstratives and interrogatives
    'ten', 'ta', 'to', 'toho', 'té', 'tomu', 'tom', 'tím',
    'tento', 'tato', 'toto', 'tamten',
    'kdo', 'koho', 'komu', 'kom', 'kým', 'co', 'čeho', 'čemu', 'čem', 'čím',
    'jaký', 'jaká', 'jaké', 'který', 'která', 'které', 'čí',
    'kde', 'kam', 'odkud', 'proč', 'kolik',
    'všechno', 'všichni', 'všechny', 'celý', 'celá', 'celé',
    # high-frequency forms of být (to be)
    'být', 'je', 'jsou', 'jsem', 'jsi', 'jsme', 'jste',
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dictionary',
        type=Path,
        default=PROJECT_ROOT / 'data' / 'dictionary-cs.db',
        help='Czech dictionary database built by scripts/build-dictionary.ts',
    )
    parser.add_argument(
        '--cache-dir',
        type=Path,
        default=PROJECT_ROOT / 'tmp' / 'cloze-cs',
        help='Tatoeba download/decompression cache',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT / 'api' / 'src' / 'lib' / 'sentence-bank-cs.json',
...
def build_candidates(dictionary_path: Path, max_words: int) -> list[Candidate]:
    if not dictionary_path.exists():
        raise FileNotFoundError(
            f'{dictionary_path} does not exist; run the Czech dictionary build first'
        )
    ...
    try:
        ...
        for raw_word in top_n_list('cs', max(max_words * 5, 10000)):
            word = normalize(raw_word).lower()
            if (
                word in seen
                or word in AVOID_WORDS
                or len(word) < 2
                or not CZECH_WORD.fullmatch(word)
...
def load_links(path: Path, sentence_ids: set[int]) -> tuple[dict[int, list[int]], set[int]]:
    ...
    with path.open(encoding='utf-8') as source:
        for line in source:
            ...
            if len(parts) < 2:
                continue
            czech_id, english_id = int(parts[0]), int(parts[1])
            if czech_id not in sentence_ids:
                continue
            links[czech_id].append(english_id)
...
def collect_options(
    ...
    for sentence_id in sorted(czech):
        text = czech[sentence_id]
...
def main() -> int:
    ...
    print('=== Czech Tatoeba cloze builder ===')
    candidates = build_candidates(args.dictionary, args.max_words)
    czech_path, links_path, english_path = ensure_tatoeba_files(args.cache_dir)

    czech = load_sentences(czech_path)
    links, needed_english = load_links(links_path, set(czech))
    english = load_english(english_path, needed_english)
    print(
        f'Tatoeba: {len(czech)} length-filtered Czech sentences; '
        f'{len(links)} linked; {len(english)} English translations'
    )

    options = collect_options(czech, links, english, candidates)
