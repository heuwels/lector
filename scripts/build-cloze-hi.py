#!/usr/bin/env python3
"""Build the Hindi Tatoeba cloze bank used by the API.

The output is derived from Tatoeba's `hin` -> `eng` per_language exports
...
Hindi is caseless. Keys are NFC only, so the two nukta spellings collapse
the same way the runtime fold does.

Prerequisites:
    pip install wordfreq
    npx tsx scripts/build-dictionary.ts --lang hi

Usage:
    python scripts/build-cloze-hi.py
    python scripts/build-cloze-hi.py --max-words 2000 --sentences-per-word 6

Downloads are cached in tmp/cloze-hi. Re-running with the same Tatoeba exports,
...
# Devanagari letters and marks. Digits and the danda are boundaries. ZWJ and
# ZWNJ stay so a conjunct that writes them can still be a candidate.
HINDI_WORD = re.compile(r'^[\u0900-\u0963\u0971-\u097F\u200C\u200D]+$')
APOSTROPHES = re.compile(r"['‘’ʼ`]+")

# Keep this aligned with languages/hi/manifest.ts. The POS filter removes most
...
AVOID_WORDS = {
    'का', 'के', 'की', 'को', 'में', 'से', 'पर', 'तक', 'ने', 'लिए', 'साथ',
    'बिना', 'द्वारा', 'ओर', 'तरफ', 'पास', 'बाद', 'पहले', 'नीचे', 'ऊपर',
    'बाहर', 'अंदर', 'बीच', 'सामने', 'पीछे',
    'मैं', 'मुझे', 'मुझ', 'मेरा', 'मेरी', 'मेरे',
    'तू', 'तुम', 'तुम्हें', 'तुम्हारा', 'तुम्हारी', 'तुम्हारे',
    'आप', 'आपका', 'आपकी', 'आपके',
    'वह', 'वो', 'वे', 'यह', 'ये', 'उस', 'इस', 'उन', 'इन',
    'उसे', 'इसे', 'उन्हें', 'इन्हें',
    'उसका', 'उसकी', 'उसके', 'इसका', 'इसकी', 'इसके',
    'उनका', 'उनकी', 'उनके',
    'हम', 'हमें', 'हमारा', 'हमारी', 'हमारे',
    'अपना', 'अपनी', 'अपने', 'कोई', 'कुछ', 'सब', 'सभी', 'खुद', 'स्वयं',
    'है', 'हैं', 'था', 'थी', 'थे', 'होगा', 'होगी', 'होंगे', 'होना',
    'हुआ', 'हुई', 'हुए', 'हो',
    'रहा', 'रही', 'रहे', 'चुका', 'चुकी', 'चुके',
    'जाता', 'जाती', 'जाते', 'वाला', 'वाली', 'वाले',
    'तो', 'ही', 'भी', 'न', 'नहीं', 'मत', 'ना',
    'क्या', 'कैसे', 'कौन', 'कहाँ', 'कब', 'क्यों', 'किधर',
    'कितना', 'कितनी', 'कितने', 'कैसा', 'कैसी',
    'और', 'या', 'लेकिन', 'मगर', 'कि', 'अगर', 'यदि', 'क्योंकि',
    'इसलिए', 'बल्कि', 'तथा', 'एवं', 'जब', 'तब', 'जैसे', 'वैसे',
    'बहुत', 'थोड़ा', 'थोड़ी', 'अधिक', 'कम', 'सिर्फ', 'केवल', 'ज्यादा',
    'हाँ', 'जी', 'अभी', 'अब', 'फिर', 'यहाँ', 'वहाँ', 'वहां', 'यहां',
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dictionary',
        type=Path,
        default=PROJECT_ROOT / 'data' / 'dictionary-hi.db',
        help='Hindi dictionary database built by scripts/build-dictionary.ts',
    )
    parser.add_argument(
        '--cache-dir',
        type=Path,
        default=PROJECT_ROOT / 'tmp' / 'cloze-hi',
        help='Tatoeba download/decompression cache',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT / 'api' / 'src' / 'lib' / 'sentence-bank-hi.json',
...
def build_candidates(dictionary_path: Path, max_words: int) -> list[Candidate]:
    if not dictionary_path.exists():
        raise FileNotFoundError(
            f'{dictionary_path} does not exist; run the Hindi dictionary build first'
        )
    ...
    try:
        for raw_word in top_n_list('hi', max(max_words * 5, 10000)):
            word = normalize(raw_word)
            if (
                word in seen
                or word in AVOID_WORDS
                or len(word) < 2
                or not HINDI_WORD.fullmatch(word)
...
def token_parts(raw_token: str) -> list[str]:
    ...
    clean = normalize(raw_token[start:end])
...
def collect_options(
    ...
    for sentence_id in sorted(hindi):
        text = hindi[sentence_id]
...
def main() -> int:
    ...
    print('=== Hindi Tatoeba cloze builder ===')
    candidates = build_candidates(args.dictionary, args.max_words)
    hindi_path, links_path, english_path = ensure_tatoeba_files(args.cache_dir)

    hindi = load_sentences(hindi_path)
    links, needed_english = load_links(links_path, set(hindi))
    english = load_english(english_path, needed_english)
    print(
        f'Tatoeba: {len(hindi)} length-filtered Hindi sentences; '
        f'{len(links)} linked; {len(english)} English translations'
    )

    options = collect_options(hindi, links, english, candidates)
