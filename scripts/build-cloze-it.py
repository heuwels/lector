#!/usr/bin/env python3
"""Build the Italian Tatoeba cloze bank used by the API.

The output is derived from Tatoeba's `ita` -> `eng` per-language exports
...
Prerequisites:
    ...
    npx tsx scripts/build-dictionary.ts --lang it

Usage:
    python scripts/build-cloze-it.py
    python scripts/build-cloze-it.py --max-words 2000 --sentences-per-word 6

Downloads are cached in tmp/cloze-it. Re-running with the same Tatoeba exports,
...
AVOID_WORDS = {
    ...
    'altro', 'altra', 'altri', 'altre', 'tutto', 'tutta', 'tutti', 'tutte',
    ...
    'e', 'ed', 'o', 'ma', 'però', 'che', 'chi', 'cui', 'come', 'cosa',
    'quando', 'dove', 'quanto', 'quanta', 'quanti', 'quante', 'quale', 'quali',
    'perché', 'perchè', 'se', 'non', 'sì', 'no',
    'io', 'tu', 'lui', 'lei', 'noi', 'voi', 'loro', 'mi', 'ti', 'si', 'ci', 'vi',
    'lo', 'la', 'li', 'le', 'ne', 'me', 'te', 'sé',
    'mio', 'mia', 'miei', 'mie', 'tuo', 'tua', 'tuoi', 'tue',
    'suo', 'sua', 'suoi', 'sue', 'nostro', 'nostra', 'nostri', 'nostre',
    'vostro', 'vostra', 'vostri', 'vostre',
    'è', 'sono', 'sei', 'siamo', 'siete', 'era', 'eri', 'eravamo', 'eravate', 'erano',
    'sia', 'siate', 'siano', 'fosse', 'fossimo', 'fossero', 'sarà', 'saranno',
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dictionary',
        type=Path,
        default=PROJECT_ROOT / 'data' / 'dictionary-it.db',
        help='Italian dictionary database built by scripts/build-dictionary.ts',
    )
    parser.add_argument(
        '--cache-dir',
        type=Path,
        default=PROJECT_ROOT / 'tmp' / 'cloze-it',
        help='Tatoeba download/decompression cache',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT / 'api' / 'src' / 'lib' / 'sentence-bank-it.json',
...
def build_candidates(dictionary_path: Path, max_words: int) -> list[Candidate]:
    if not dictionary_path.exists():
        raise FileNotFoundError(
            f'{dictionary_path} does not exist; run the Italian dictionary build first'
        )
    ...
    try:
        ...
        for raw_word in top_n_list('it', max(max_words * 5, 10000)):
            word = normalize(raw_word).lower()
            if (
                word in seen
                or word in AVOID_WORDS
                or len(word) < 2
                or not ITALIAN_WORD.fullmatch(word)
...
def load_links(path: Path, sentence_ids: set[int]) -> tuple[dict[int, list[int]], set[int]]:
    ...
    with path.open(encoding='utf-8') as source:
        for line in source:
            ...
            if len(parts) < 2:
                continue
            italian_id, english_id = int(parts[0]), int(parts[1])
            if italian_id not in sentence_ids:
                continue
            links[italian_id].append(english_id)
...
def collect_options(
    ...
    for sentence_id in sorted(italian):
        text = italian[sentence_id]
...
def main() -> int:
    ...
    print('=== Italian Tatoeba cloze builder ===')
    candidates = build_candidates(args.dictionary, args.max_words)
    italian_path, links_path, english_path = ensure_tatoeba_files(args.cache_dir)

    italian = load_sentences(italian_path)
    links, needed_english = load_links(links_path, set(italian))
    english = load_english(english_path, needed_english)
    print(
        f'Tatoeba: {len(italian)} length-filtered Italian sentences; '
        f'{len(links)} linked; {len(english)} English translations'
    )

    options = collect_options(italian, links, english, candidates)
