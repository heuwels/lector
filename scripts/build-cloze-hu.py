#!/usr/bin/env python3
"""Build the Hungarian Tatoeba cloze bank used by the API.

The output is derived from Tatoeba's `hun` -> `eng` per-language exports
...
Prerequisites:
    ...
    npx tsx scripts/build-dictionary.ts --lang hu

Usage:
    python scripts/build-cloze-hu.py
    python scripts/build-cloze-hu.py --max-words 2000 --sentences-per-word 6

Downloads are cached in tmp/cloze-hu. Re-running with the same Tatoeba exports,
...
HUNGARIAN_WORD = re.compile(r'^[a-záéíóöőúüű-]+$')
APOSTROPHES = re.compile(r"['‘’ʼ`]+")

# Keep this aligned with languages/hu/manifest.ts.
AVOID_WORDS = {
    'a', 'az', 'egy',
    'én', 'te', 'ő', 'mi', 'ti', 'ők',
    ...
    'van', 'vagy', 'vagyok', 'vagyunk', 'vagytok', 'vannak',
    'volt', 'voltam', 'voltál', 'voltunk', 'voltatok', 'voltak',
    'lesz', 'lenni',
    'nem', 'ne', 'nincs', 'sincs',
    'és', 'de', 'hogy', 'ha', 'mert', 'amikor', 'mint', 'hanem', 'pedig',
    'mi', 'ki', 'hol', 'mikor', 'miért', 'hogyan', 'melyik',
    'ez', 'itt', 'ott', 'most', 'már', 'még', 'csak', 'is', 'nagyon',
    'igen', 'el', 'meg', 'be', 'le', 'fel',
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dictionary',
        type=Path,
        default=PROJECT_ROOT / 'data' / 'dictionary-hu.db',
        help='Hungarian dictionary database built by scripts/build-dictionary.ts',
    )
    parser.add_argument(
        '--cache-dir',
        type=Path,
        default=PROJECT_ROOT / 'tmp' / 'cloze-hu',
        help='Tatoeba download/decompression cache',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT / 'api' / 'src' / 'lib' / 'sentence-bank-hu.json',
...
def build_candidates(dictionary_path: Path, max_words: int) -> list[Candidate]:
    if not dictionary_path.exists():
        raise FileNotFoundError(
            f'{dictionary_path} does not exist; run the Hungarian dictionary build first'
        )
    ...
    try:
        ...
        for raw_word in top_n_list('hu', max(max_words * 5, 10000)):
            word = normalize(raw_word).lower()
            if (
                word in seen
                or word in AVOID_WORDS
                or len(word) < 2
                or not HUNGARIAN_WORD.fullmatch(word)
...
def main() -> int:
    ...
    print('=== Hungarian Tatoeba cloze builder ===')
    candidates = build_candidates(args.dictionary, args.max_words)
    hungarian_path, links_path, english_path = ensure_tatoeba_files(args.cache_dir)

    hungarian = load_sentences(hungarian_path)
    links, needed_english = load_links(links_path, set(hungarian))
    english = load_english(english_path, needed_english)
    print(
        f'Tatoeba: {len(hungarian)} length-filtered Hungarian sentences; '
        f'{len(links)} linked; {len(english)} English translations'
    )

    options = collect_options(hungarian, links, english, candidates)
