#!/usr/bin/env python3
"""Build the Finnish Tatoeba cloze bank used by the API.

The output is derived from Tatoeba's `fin` -> `eng` per-language exports
...
Prerequisites:
    ...
    npx tsx scripts/build-dictionary.ts --lang fi

Usage:
    python scripts/build-cloze-fi.py
    python scripts/build-cloze-fi.py --max-words 2000 --sentences-per-word 6

Downloads are cached in tmp/cloze-fi. Re-running with the same Tatoeba exports,
...
FINNISH_WORD = re.compile(r'^[a-zäö-]+$')
APOSTROPHES = re.compile(r"['‘’ʼ`]+")

# Keep this aligned with languages/fi/manifest.ts.
AVOID_WORDS = {
    ...
    'tämä', 'tuo', 'nämä', 'nuo', 'tämän', 'tuon', 'näiden', 'noiden',
    'olla', 'olen', 'olet', 'on', 'olemme', 'olette', 'ovat',
    'oli', 'olivat', 'ole', 'ollut', 'olisi',
    'ei', 'en', 'et', 'emme', 'ette', 'eivät', 'älä', 'älkää',
    'ja', 'tai', 'mutta', 'että', 'kun', 'jos', 'koska', 'vaikka',
    'kuin', 'niin', 'sekä',
    'myös', 'vain', 'jo', 'vielä', 'nyt', 'sitten', 'tässä', 'siinä',
    'siellä', 'täällä', 'siis', 'kyllä', 'joo',
    'mikä', 'mitä', 'kuka', 'ken', 'missä', 'mistä', 'mihin',
    'milloin', 'miksi', 'miten', 'kuinka',
    'kaikki', 'joku', 'jokin', 'muu', 'yksi', 'kaksi',
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dictionary',
        type=Path,
        default=PROJECT_ROOT / 'data' / 'dictionary-fi.db',
        help='Finnish dictionary database built by scripts/build-dictionary.ts',
    )
    parser.add_argument(
        '--cache-dir',
        type=Path,
        default=PROJECT_ROOT / 'tmp' / 'cloze-fi',
        help='Tatoeba download/decompression cache',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT / 'api' / 'src' / 'lib' / 'sentence-bank-fi.json',
...
def build_candidates(dictionary_path: Path, max_words: int) -> list[Candidate]:
    if not dictionary_path.exists():
        raise FileNotFoundError(
            f'{dictionary_path} does not exist; run the Finnish dictionary build first'
        )
    ...
    try:
        ...
        for raw_word in top_n_list('fi', max(max_words * 5, 10000)):
            word = normalize(raw_word).lower()
            if (
                word in seen
                or word in AVOID_WORDS
                or len(word) < 2
                or not FINNISH_WORD.fullmatch(word)
...
def main() -> int:
    ...
    print('=== Finnish Tatoeba cloze builder ===')
    candidates = build_candidates(args.dictionary, args.max_words)
    finnish_path, links_path, english_path = ensure_tatoeba_files(args.cache_dir)

    finnish = load_sentences(finnish_path)
    links, needed_english = load_links(links_path, set(finnish))
    english = load_english(english_path, needed_english)
    print(
        f'Tatoeba: {len(finnish)} length-filtered Finnish sentences; '
        f'{len(links)} linked; {len(english)} English translations'
    )

    options = collect_options(finnish, links, english, candidates)
