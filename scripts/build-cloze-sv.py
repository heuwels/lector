#!/usr/bin/env python3
"""Build the Swedish Tatoeba cloze bank used by the API.

The output is derived from Tatoeba's `swe` -> `eng` per-language exports
...
Prerequisites:
    ...
    npx tsx scripts/build-dictionary.ts --lang sv

Usage:
    python scripts/build-cloze-sv.py
    python scripts/build-cloze-sv.py --max-words 2000 --sentences-per-word 6

Downloads are cached in tmp/cloze-sv. Re-running with the same Tatoeba exports,
...
SWEDISH_WORD = re.compile(r'^[a-zåäö-]+$')
APOSTROPHES = re.compile(r"['‘’ʼ`]+")

# Keep this aligned with languages/sv/manifest.ts.
AVOID_WORDS = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dictionary',
        type=Path,
        default=PROJECT_ROOT / 'data' / 'dictionary-sv.db',
        help='Swedish dictionary database built by scripts/build-dictionary.ts',
    )
    parser.add_argument(
        '--cache-dir',
        type=Path,
        default=PROJECT_ROOT / 'tmp' / 'cloze-sv',
        help='Tatoeba download/decompression cache',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT / 'api' / 'src' / 'lib' / 'sentence-bank-sv.json',
...
def build_candidates(dictionary_path: Path, max_words: int) -> list[Candidate]:
    if not dictionary_path.exists():
        raise FileNotFoundError(
            f'{dictionary_path} does not exist; run the Swedish dictionary build first'
        )
    ...
    try:
        ...
        for raw_word in top_n_list('sv', max(max_words * 5, 10000)):
            word = normalize(raw_word).lower()
            if (
                word in seen
                or word in AVOID_WORDS
                or len(word) < 2
                or not SWEDISH_WORD.fullmatch(word)
...
def main() -> int:
    ...
    print('=== Swedish Tatoeba cloze builder ===')
    candidates = build_candidates(args.dictionary, args.max_words)
    swedish_path, links_path, english_path = ensure_tatoeba_files(args.cache_dir)

    swedish = load_sentences(swedish_path)
    links, needed_english = load_links(links_path, set(swedish))
    english = load_english(english_path, needed_english)
    print(
        f'Tatoeba: {len(swedish)} length-filtered Swedish sentences; '
        f'{len(links)} linked; {len(english)} English translations'
    )

    options = collect_options(swedish, links, english, candidates)
