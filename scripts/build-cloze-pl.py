#!/usr/bin/env python3
"""Build the Polish Tatoeba cloze bank used by the API.

The output is derived from Tatoeba's `pol` -> `eng` per-language exports
...
Prerequisites:
    ...
    npx tsx scripts/build-dictionary.ts --lang pl

Usage:
    python scripts/build-cloze-pl.py
    python scripts/build-cloze-pl.py --max-words 2000 --sentences-per-word 6

Downloads are cached in tmp/cloze-pl. Re-running with the same Tatoeba exports,
...
# Plain Polish letter runs only: a-z plus the nine diacritic letters. Hyphenated
# compounds (biało-czerwony, polsko-angielski) stay whole tokens at runtime and
# in sentence_matches below, so they can never collide with a plain candidate;
# wordfreq emits unhyphenated tokens anyway.
POLISH_WORD = re.compile(r'^[a-ząćęłńóśźż]+$')
APOSTROPHES = re.compile(r"['‘’ʼ`]+")

# Keep this aligned with languages/pl/manifest.ts. The POS filter removes most
...
AVOID_WORDS = {
    # prepositions
    'w', 'we', 'z', 'ze', 'na', 'do', 'od', 'ode', 'dla', 'przez', 'po',
    'przy', 'o', 'u', 'za', 'pod', 'nad', 'przed', 'między', 'bez', 'oprócz',
    'obok', 'około', 'wobec', 'według', 'podczas', 'mimo', 'ku', 'wśród',
    # conjunctions and connectives
    'i', 'a', 'ale', 'lecz', 'oraz', 'czy', 'że', 'żeby', 'aby', 'jeśli',
    'jeżeli', 'gdy', 'kiedy', 'bo', 'ponieważ', 'więc', 'także', 'też',
    'jednak', 'choć', 'chociaż', 'albo', 'lub', 'ani', 'niż',
    # particles
    'nie', 'się', 'by', 'no', 'oto', 'już', 'jeszcze', 'tylko', 'bardzo',
    'nawet', 'tak', 'właśnie', 'może', 'chyba', 'niech',
    # personal, reflexive and possessive pronouns (case forms)
    'ja', 'ty', 'on', 'ona', 'ono', 'my', 'wy', 'oni', 'one',
    'mnie', 'mi', 'mną', 'ciebie', 'cię', 'tobie', 'ci', 'tobą',
    'jego', 'go', 'jemu', 'mu', 'nim', 'niego',
    'jej', 'ją', 'niej', 'nią',
    ...
    'ten', 'ta', 'to', 'te', 'tego', 'tej', 'tym', 'temu',
    'tamten', 'tamta', 'tamto',
    'kto', 'kogo', 'komu', 'kim', 'co', 'czego', 'czemu', 'czym',
    'jaki', 'jaka', 'jakie', 'który', 'która', 'które', 'czyj',
    'gdzie', 'dokąd', 'skąd', 'dlaczego', 'ile',
    'wszystko', 'wszyscy', 'wszystkie', 'cały', 'cała', 'całe',
    # high-frequency forms of być (to be)
    'być', 'jest', 'są', 'jestem', 'jesteś', 'jesteśmy', 'jesteście',
    'był', 'była', 'było', 'byli', 'były',
    'będzie', 'będą', 'będę', 'będziesz', 'będziemy',
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dictionary',
        type=Path,
        default=PROJECT_ROOT / 'data' / 'dictionary-pl.db',
        help='Polish dictionary database built by scripts/build-dictionary.ts',
    )
    parser.add_argument(
        '--cache-dir',
        type=Path,
        default=PROJECT_ROOT / 'tmp' / 'cloze-pl',
        help='Tatoeba download/decompression cache',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT / 'api' / 'src' / 'lib' / 'sentence-bank-pl.json',
...
def build_candidates(dictionary_path: Path, max_words: int) -> list[Candidate]:
    if not dictionary_path.exists():
        raise FileNotFoundError(
            f'{dictionary_path} does not exist; run the Polish dictionary build first'
        )
    ...
    try:
        ...
        for raw_word in top_n_list('pl', max(max_words * 5, 10000)):
            word = normalize(raw_word).lower()
            if (
                word in seen
                or word in AVOID_WORDS
                or len(word) < 2
                or not POLISH_WORD.fullmatch(word)
...
def load_links(path: Path, sentence_ids: set[int]) -> tuple[dict[int, list[int]], set[int]]:
    ...
    with path.open(encoding='utf-8') as source:
        for line in source:
            ...
            if len(parts) < 2:
                continue
            polish_id, english_id = int(parts[0]), int(parts[1])
            if polish_id not in sentence_ids:
                continue
            links[polish_id].append(english_id)
...
def collect_options(
    ...
    for sentence_id in sorted(polish):
        text = polish[sentence_id]
...
def main() -> int:
    ...
    print('=== Polish Tatoeba cloze builder ===')
    candidates = build_candidates(args.dictionary, args.max_words)
    polish_path, links_path, english_path = ensure_tatoeba_files(args.cache_dir)

    polish = load_sentences(polish_path)
    links, needed_english = load_links(links_path, set(polish))
    english = load_english(english_path, needed_english)
    print(
        f'Tatoeba: {len(polish)} length-filtered Polish sentences; '
        f'{len(links)} linked; {len(english)} English translations'
    )

    options = collect_options(polish, links, english, candidates)
