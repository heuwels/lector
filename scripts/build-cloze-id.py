#!/usr/bin/env python3
"""Build the Indonesian Tatoeba cloze bank used by the API.

The output is derived from Tatoeba's `ind` -> `eng` per-language exports
...
Prerequisites:
    ...
    npx tsx scripts/build-dictionary.ts --lang id

Usage:
    python scripts/build-cloze-id.py
    python scripts/build-cloze-id.py --max-words 2000 --sentences-per-word 6

Downloads are cached in tmp/cloze-id. Re-running with the same Tatoeba exports,
...
# Plain Latin plus hyphen for reduplicated plurals (buku-buku). Official
# spelling has no diacritics.
INDONESIAN_WORD = re.compile(r'^[a-z-]+$')
APOSTROPHES = re.compile(r"['‘’ʼ`]+")

# Keep this aligned with languages/id/manifest.ts. The POS filter removes most
...
AVOID_WORDS = {
    'saya', 'aku', 'kamu', 'anda', 'kau', 'engkau', 'dia', 'ia', 'beliau',
    'kita', 'kami', 'mereka', 'ini', 'itu', 'tersebut', 'nya', 'ku', 'mu',
    'lah', 'kah', 'pun',
    'tidak', 'bukan', 'jangan', 'belum', 'tanpa',
    'di', 'ke', 'dari', 'untuk', 'dengan', 'pada', 'oleh', 'tentang',
    'kepada', 'daripada', 'terhadap', 'bagi', 'dalam', 'atas', 'antara',
    'dan', 'atau', 'tetapi', 'tapi', 'karena', 'sebab', 'jika', 'kalau',
    'walaupun', 'meskipun', 'sehingga', 'lalu', 'kemudian', 'serta', 'yang',
    'bahwa',
    'adalah', 'ialah', 'ada', 'yaitu', 'yakni',
    'sudah', 'telah', 'sedang', 'akan', 'ingin', 'mau', 'bisa', 'dapat',
    'harus', 'boleh', 'hendak', 'pernah', 'masih',
    'sini', 'situ', 'sana', 'apa', 'siapa', 'mana', 'kapan', 'mengapa',
    'kenapa', 'bagaimana', 'berapa',
    'juga', 'hanya', 'saja', 'sangat', 'lebih', 'paling', 'lagi', 'begitu',
    'demikian', 'seperti', 'sebagai',
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--dictionary',
        type=Path,
        default=PROJECT_ROOT / 'data' / 'dictionary-id.db',
        help='Indonesian dictionary database built by scripts/build-dictionary.ts',
    )
    parser.add_argument(
        '--cache-dir',
        type=Path,
        default=PROJECT_ROOT / 'tmp' / 'cloze-id',
        help='Tatoeba download/decompression cache',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT / 'api' / 'src' / 'lib' / 'sentence-bank-id.json',
...
def build_candidates(dictionary_path: Path, max_words: int) -> list[Candidate]:
    if not dictionary_path.exists():
        raise FileNotFoundError(
            f'{dictionary_path} does not exist; run the Indonesian dictionary build first'
        )
    ...
    try:
        ...
        for raw_word in top_n_list('id', max(max_words * 5, 10000)):
            word = normalize(raw_word).lower()
            if (
                word in seen
                or word in AVOID_WORDS
                or len(word) < 2
                or not INDONESIAN_WORD.fullmatch(word)
...
def collect_options(
    ...
    for sentence_id in sorted(indonesian):
        text = indonesian[sentence_id]
...
def main() -> int:
    ...
    print('=== Indonesian Tatoeba cloze builder ===')
    candidates = build_candidates(args.dictionary, args.max_words)
    indonesian_path, links_path, english_path = ensure_tatoeba_files(args.cache_dir)

    indonesian = load_sentences(indonesian_path)
    links, needed_english = load_links(links_path, set(indonesian))
    english = load_english(english_path, needed_english)
    print(
        f'Tatoeba: {len(indonesian)} length-filtered Indonesian sentences; '
        f'{len(links)} linked; {len(english)} English translations'
    )

    options = collect_options(indonesian, links, english, candidates)
