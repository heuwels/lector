...
test.describe('Translation drawer — dictionary lookup pipeline', () => {
  ...
  test.afterEach(async ({ page }) => {
    await page.request.delete(`http://localhost:3456/api/cloze/${TEST_SENTENCE_ID}`);
  });
  ...
  test('local-dict miss falls back to AI translate', async ({ page }) => {
    ...
    await page.request.post('http://localhost:3456/api/cloze', { data: [missSentence] });
    ...
    try {
      ...
      const lookupRes = await page.request.get(
        'http://localhost:3456/api/dictionary/lookup?word=xyznonexistent'
      );
      ...
      await page.request.delete(`http://localhost:3456/api/cloze/test-drawer-miss-1`);
    }
  });
