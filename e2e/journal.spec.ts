...
test.describe("Journal", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const res = await page.request.get("http://localhost:3457/api/journal?limit=100");
    const entries = await res.json();
    for (const e of entries) {
      if (e.entryDate.startsWith(TEST_PREFIX)) {
        await page.request.delete(`http://localhost:3457/api/journal/${e.id}`);
      }
    }
  });
  ...
  test("should create and save a draft entry via API", async ({ page }) => {
    const createRes = await page.request.post("http://localhost:3457/api/journal", {
    ...
    const getRes = await page.request.get(`http://localhost:3457/api/journal/${id}`);
    const entry = await getRes.json();
    expect(entry.body).toBe("Dit is 'n toets inskrywing.");
    expect(entry.status).toBe("draft");
    expect(entry.wordCount).toBe(5);

    await page.request.delete(`http://localhost:3457/api/journal/${id}`);
  });

  test("should allow multiple entries per day", async ({ page }) => {
    const res1 = await page.request.post("http://localhost:3457/api/journal", {
      data: { body: "Eerste inskrywing.", entryDate: "2099-01-01" },
    });
    const res2 = await page.request.post("http://localhost:3457/api/journal", {
    ...
    const listRes = await page.request.get("http://localhost:3457/api/journal?date=2099-01-01");
    const entries = await listRes.json();
    expect(entries.length).toBe(2);

    await page.request.delete(`http://localhost:3457/api/journal/${id1}`);
    await page.request.delete(`http://localhost:3457/api/journal/${id2}`);
  });
  ...
  test("should delete an entry", async ({ page }) => {
    ...
    const deleteRes = await page.request.delete(`http://localhost:3457/api/journal/${id}`);
  ...
  test("should save draft via UI and show in history", async ({ page }) => {
    ...
    const res = await page.request.get(`http://localhost:3457/api/journal?date=${today}`);
    const entries = await res.json();
    for (const e of entries) {
  ...
  test("full journey: create, save draft, navigate away, return, submit", async ({
    ...
    const existing = await page.request.get(`http://localhost:3457/api/journal?date=${today}`);
    ...
    // Clean up
    const apiRes = await page.request.get(`http://localhost:3457/api/journal?date=${today}`);
