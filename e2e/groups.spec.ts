...
test.describe("Collection Groups", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const groupsRes = await page.request.get("http://localhost:3457/api/groups");
    const groups = await groupsRes.json();
    for (const g of groups) {
      if (g.name.startsWith("Test") || g.name.startsWith("Toets")) {
        await page.request.delete(`http://localhost:3457/api/groups/${g.id}`);
      }
    }

    // Clean up test collections
    const colRes = await page.request.get("http://localhost:3457/api/collections");
  ...
  test("should assign a collection to a group via detail page", async ({ page }) => {
    ...
    const groupRes = await page.request.post("http://localhost:3457/api/groups", {
  ...
  test("should ungroup a collection", async ({ page }) => {
    ...
    await page.request.put(`http://localhost:3457/api/collections/${collectionId}`, {
      data: { groupId },
    });
  ...
  test("groups API CRUD works correctly", async ({ page }) => {
    // POST
    const createRes = await page.request.post("http://localhost:3457/api/groups", {
    ...
    // GET
    const listRes = await page.request.get("http://localhost:3457/api/groups");
    ...
    // PUT
    const updateRes = await page.request.put(`http://localhost:3457/api/groups/${id}`, {
      data: { name: "Test API Group Renamed" },
    });
    expect(updateRes.ok()).toBeTruthy();

    const listRes2 = await page.request.get("http://localhost:3457/api/groups");
    const groups2 = await listRes2.json();
    expect(groups2.find((g: { id: string }) => g.id === id).name).toBe("Test API Group Renamed");

    // DELETE
    const deleteRes = await page.request.delete(`http://localhost:3457/api/groups/${id}`);
    expect(deleteRes.ok()).toBeTruthy();

    const listRes3 = await page.request.get("http://localhost:3457/api/groups");
  ...
  test("should reject empty group name", async ({ page }) => {
    const res = await page.request.post("http://localhost:3457/api/groups", {
      data: { name: "" },
    });
